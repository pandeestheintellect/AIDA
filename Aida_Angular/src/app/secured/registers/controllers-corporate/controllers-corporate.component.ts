...
@Component({
  selector: 'app-controllers-corporate',
  templateUrl: './controllers-corporate.component.html',
  styles: []
})
export class ControllersCorporateComponent implements OnInit {

  pageTitle = "Register of Registrable Controllers (For Corporate Entities)";
  addToolTip = "Add  new Controllers details";
  filterCaption = "Search in Controllers details";
  ...
  rootPath="api/register/corporate-register";

  displayedColumns: string[] = ['entryDate','name','registrationNo','legalForm','startDate','cessationDate','toolbox'];
  ...
  OnAdd()
  {
    ...
    legalForm: '',jurisdiction: '',statue: '',identityNo:'',registerName: '',businessProfileId:this.clientProfileId});
  }
  OnEdit(option,paramData) {
    const dialogRef = this.dialog.open(ControllersCorporateDetailComponent, {
      width: '1200px',
  ...
  OnCustom()
  {
    this.resourceClient.getDataInGet('api/register/download-corporate-register'+'/'+this.clientProfileId).subscribe((data: any)=>{
